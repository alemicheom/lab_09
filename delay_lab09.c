 ...
 * Author: aleja
 *
 * Created on April 26, 2022, 10:43 AM
 */
...
void __interrupt() isr (void){
    if(PIR1bits.ADIF){                      // interrupción del ADC
        if(ADCON0bits.CHS == 0){            // AN0 el canal seleccionado
        ...
        else if(ADCON0bits.CHS == 1){            // AN1 el canal seleccionado
            CCPR_2 = map(ADRESH, IN_MIN, IN_MAX, OUT_MIN, OUT_MAX); //  ancho de pulso
            CCPR2L = (uint8_t)(CCPR_2>>2);    // Guardamos los 8 bits mas significativos en CPR2L
            CCP2CONbits.DC2B0 = CCPR_2 & 0b11; // Guardamos los 2 bits menos significativos en DC2B0
            ADCON0bits.CHS = 2;
            
        }
        
        else if(ADCON0bits.CHS == 2){            // AN2 el canal seleccionado
            ADCON0bits.CHS = 0;
        }
